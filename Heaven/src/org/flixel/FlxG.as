package org.flixel
{
	...
	public class FlxG
	{
		...
		//==============================================
		static public function addBitmapData(bitmapData:BitmapData, Reverse:Boolean=false, Unique:Boolean=false, Key:String=null):BitmapData
		{
			var needReverse:Boolean = false;
			var key:String = Key;
			if(key == null)
		...
		/**
		 * Function that force the camera and parallax scrolling to follow the target immediately
		 */
		static public function forceFollow():void {
			if(followTarget != null)
			{
				_scrollTarget.x = (width>>1)-followTarget.x-(followTarget.width>>1);
				_scrollTarget.y = (height>>1)-followTarget.y-(followTarget.height>>1);
